...
public class UCApi {
    ...
    private static class HttpResult {
        final int code;
        final String body;
        final Map<String, List<String>> resp;
        HttpResult(int code, String body, Map<String, List<String>> resp) {
            this.code = code;
            this.body = body;
            this.resp = resp;
        }
    ...
    private static HttpResult doGet(String url, Map<String, String> params, Map<String, String> headers) throws Exception {
        ...
        if (headers != null) reqBuilder.headers(okhttp3.Headers.of(headers));
        okhttp3.Response resp = OkHttp.client().newCall(reqBuilder.build()).execute();
        int code = resp.code();
        String body = resp.body().string();
        Map<String, List<String>> respHeaders = resp.headers().toMultimap();
        resp.close();
        return new HttpResult(code, body, respHeaders);
    }

    private static HttpResult doPost(String url, String json, Map<String, String> headers) throws Exception {
        MediaType mediaType = MediaType.parse("application/json; charset=utf-8");
        RequestBody body = RequestBody.create(json, mediaType);
        okhttp3.Response resp = OkHttp.newCall(url, headers, body).execute();
        int code = resp.code();
        String bodyStr = resp.body().string();
        Map<String, List<String>> respHeaders = resp.headers().toMultimap();
        resp.close();
        return new HttpResult(code, bodyStr, respHeaders);
    }
    ...
    private void initUserInfo() {
        try {
            ...
            HttpResult result = doGet("https://drive.uc.cn/account/info?st=" + token + "", new HashMap<>(), getWebHeaders());
            Map json = Json.parseSafe(result.body, Map.class);
            if (json.get("success").equals(Boolean.TRUE)) {
                List<String> cookies = result.resp.get("set-Cookie");
            ...
            cache.getUser().clean();
            e.printStackTrace();
            SpiderDebug.log("Failed to init user info: cookie required");
        }
    }
    ...
    public List<String> getPlayFormatUcList() {
        return new ArrayList<>();
    }

    public File getCache() {
        return Path.cache("uc");
    }
            ...
            for (int index = 0; index < playFromtmp.size(); index++) {
                ...
                for (Item video_item : files) {
                    if (video_item.getShareIndex() == i + 1) {
                        vodItems.add(video_item.getEpisodeUrl("电影"));
                    }
                }
    ...
    private boolean testVideo(String url) {
        try {
            HttpResult result = doGet(url, new HashMap<>(), Map.of("Range", "bytes=0-0"));
    ...
    private Object[] getM3u8(String url, Map header) {
        try {
            ...
            HttpResult result = doGet(url, new HashMap<>(), header);
            String[] m3u8Arr = result.body.split("\n");
            ...
            String contentType = result.resp.get("Content-Type").get(0);

            Map<String, String> respHeaders = new HashMap<>();
            for (String key : result.resp.keySet()) {
                respHeaders.put(key, result.resp.get(key).get(0));
            }
            return new Object[]{result.code, contentType, new ByteArrayInputStream(m3u8Str.getBytes(Charset.defaultCharset())), respHeaders};
        } catch (Exception e) {
            return new Object[]{500, "text/plain", new ByteArrayInputStream("".getBytes()), new HashMap<>()};
        }
    }
    ...
    private String api(String url, Map<String, String> params, Map<String, Object> data, Integer retry, String method) throws Exception {
        ...
        if ("GET".equals(method)) {
            okResult = doGet(this.apiUrl + url, params, getHeaders());
        } else {
            okResult = doPost(this.apiUrl + url, Json.toJson(data), getHeaders());
        }
        if (okResult.resp.get("Set-Cookie") != null) {
            Matcher matcher = Pattern.compile("__puus=([^;]+)").matcher(StringUtils.join(okResult.resp.get("Set-Cookie"), ";;;"));
            if (matcher.find()) {
        ...
        if (okResult.code != 200 && leftRetry > 0) {
            SpiderDebug.log("api error code:" + okResult.code);
            Thread.sleep(1000);
            return api(url, params, data, leftRetry - 1, method);
        }
    ...
    public static class LCSResult {
        ...
        public LCSResult(int length, String sequence, int index) {
            this.length = length;
    ...
    public void getFilesByShareUrl(int shareIndex, String shareInfo, List<Item> videos, List<Item> subtitles) throws Exception {
        ShareData shareData = getShareData(shareInfo);
