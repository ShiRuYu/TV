...
public class Dm84 extends Spider {
    ...
    public String detailContent(List<String> ids) {
        ...
        for (int i = 0; i < sources.size(); i++) {
            ...
            if (vodItems.size() > 0) {
                sites.put(sourceName, String.join("#", vodItems));
            }
        }
        if (sites.size() > 0) {
            vod.setVodPlayFrom(String.join("$$$", new ArrayList<>(sites.keySet())));
            vod.setVodPlayUrl(String.join("$$$", new ArrayList<>(sites.values())));
        }
    ...
    public String playerContent(String flag, String id, List<String> vipFlags) {
        ...
        String post = OkHttp.post(playUrl.concat("/api.php"), map, Util.webHeaders(urlLink));
