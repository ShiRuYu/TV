...
public class Supjav extends Spider {
    ...
    private String parseTV(String redirect) throws MalformedURLException {
        String data = OkHttp.string(redirect, getTVVideoHeaders(new URL(redirect).getHost()));
        return Result.get().url(Util.getVar(data, "urlPlay")).header(getTVVideoHeaders(new URL(redirect).getHost())).string();
    }
    ...
    private String parseDS(String redirect) throws URISyntaxException, IOException {
        String host = "https://" + new URI(redirect).getHost();
