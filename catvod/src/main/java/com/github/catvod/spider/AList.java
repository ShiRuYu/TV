...
public class AList extends Spider {
    ...
    private String post(Drive drive, String url, String param, boolean retry) {
        String response = OkHttp.post(url, param.toString(), drive.getHeader());
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) throws Exception {
        ...
        if (!StringUtils.isEmpty(type) && !StringUtils.isEmpty(order)) {
    ...
    private boolean login(Drive drive) {
        try {
            ...
            String response = OkHttp.post(drive.loginApi(), params.toString(), null);
    ...
    private String findSubs(String path, List<Item> items) {
        ...
        for (Item item : items)
            if (item.getExt() != null && (item.getExt().contains("srt") || item.getExt().contains("vtt") || item.getExt().contains("ass")))
                sb.append("~~~").append(item.getName()).append("@@@").append(item.getExt()).append("@@@").append(item.getVodId(path));
        return sb.toString();
    }
    ...
    class Job implements Callable<List<Vod>> {
        ...
        private List<Vod> xiaoya() {
            ...
            for (Element a : doc.select("ul > a")) {
                ...
                boolean file = splits[0] != null && (splits[0].contains("mp4") || splits[0].contains("mkv") || splits[0].contains("flv") || splits[0].contains("ts"));
                Item item = new Item();
